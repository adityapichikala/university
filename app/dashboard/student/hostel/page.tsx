...
import { HostelLeavePanel } from './HostelLeavePanel'
...
export default async function StudentHostelPage() {
  ...
  const [allocations, leaves] = await Promise.all([
    ...
    // Newest first — the student cares about the request they just filed.
          ...
          <HostelLeavePanel
            student={{
              ...
              roomLabel: current
                ? roomLabel(current.room.block, current.room.roomNumber)
                : null,
              block: current ? String(current.room.block) : null,
            }}
            leaves={leaveRows}
          />
        </div>
